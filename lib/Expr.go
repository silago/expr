...
func (e *Expression) GetVariableNames() (result []string) {
	fmt.Println(e.Left)
	l := e.Left.GetVariableNames()
	result = append(result, l...)
	for _, r := range e.Right {
		//result = append(result, r.Operator.GetVariableNames()...)
		//result = append(result, l.GetVariableNames()...)
		result = append(result, r.Term.GetVariableNames()...)
		//, r.Term.Eval(ctx))
	}
	return
}
...
func ExtractVariables(str string) (result []string, err error) {
	expr := &Expression{}
	if parser, err := participle.Build(&Expression{}); err != nil {
		...
		var result = expr.GetVariableNames()
		return result, nil
	}
	return
}
